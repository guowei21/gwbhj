...
void SHA256::transform(State* ctx, const uint8_t block[BLOCK_SIZE]) {
    uint32_t W[64];
    for (int i = 0; i < 16; i++) {
...
void SHA256::update(State* ctx, const uint8_t* data, size_t len) {
    for (size_t i = 0; i < len; i++) {
        ctx->buffer[ctx->buflen++] = data[i];
        if (ctx->buflen == BLOCK_SIZE) {
            transform(ctx, ctx->buffer);
            ctx->bitcount += BLOCK_SIZE * 8;
            ctx->buflen = 0;
        }
    }
}

void SHA256::finalize(State* ctx, uint8_t digest[DIGEST_SIZE]) {
    ctx->bitcount += ctx->buflen * 8;

    ctx->buffer[ctx->buflen++] = 0x80;

    if (ctx->buflen > 56) {
        while (ctx->buflen < BLOCK_SIZE) ctx->buffer[ctx->buflen++] = 0;
        transform(ctx, ctx->buffer);
        ctx->buflen = 0;
    }
...
std::vector<uint8_t> SHA256::hashBytes(const uint8_t* data, size_t len) {
    State ctx;
    init(&ctx);
    update(&ctx, data, len);
