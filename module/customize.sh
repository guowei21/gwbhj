...
# ── regenerate serial.txt (force update on every flash) ──────────
SERIAL=$(cat /dev/urandom | tr -dc 'A-Za-z0-9' | head -c 16 2>/dev/null)
if [ -z "$SERIAL" ]; then
    SERIAL="HW$(date +%s%N | md5sum | head -c 14)"
fi
echo "$SERIAL" > "$MODPATH/serial.txt"
chmod 644 "$MODPATH/serial.txt" 2>/dev/null
ui_print " Serial: $SERIAL"

# ── regenerate android_id.txt (force update on every flash) ──────
AID=$(cat /dev/urandom | tr -dc '0-9' | head -c 19 2>/dev/null)
if [ -z "$AID" ]; then
    AID="$(date +%s%N | head -c 19)"
fi
echo "$AID" > "$MODPATH/android_id.txt"
chmod 644 "$MODPATH/android_id.txt" 2>/dev/null
ui_print " ANDROID_ID seed: $AID"
...
set_permissions() {
    ...
    chmod 0644 "$MODPATH/module.prop" "$MODPATH/whitelist.txt" "$MODPATH/freeze_list.txt" "$MODPATH/serial.txt" "$MODPATH/android_id.txt" 2>/dev/null
...
# ── auto soft reboot via KSU manager ──────────────────────────
ui_print " Triggering soft reboot..."
if command -v ksud >/dev/null 2>&1; then
    ksud soft-reboot
elif [ -f /data/adb/ksud ]; then
    /data/adb/ksud soft-reboot
else
    ui_print " ksud not found, please reboot manually."
fi
